# test/ref/starship_sh_ref.sh — the sh implementation of modules/starship.sh, FROZEN.
#
# The last pure-sh version, kept as the specification of what the C module
# (modules/starship.c) must do: test/unit/module_c_parity.sh runs both under
...
if [ -f "$_ss_base" ]; then
    _ss_pal=$(osr_theme_source starship starship.palette.toml) || _ss_pal=""
    if [ -n "$_ss_pal" ]; then
        compose_starship_config "$_ss_base" "$_ss_pal" "$_ss_dst"
        case "$_ss_pal" in "${TMPDIR:-/tmp}"/osr-theme-*) rm -f "$_ss_pal" ;; esac
    else
