# test/ref/serie_sh_ref.sh — the sh implementation of modules/serie.sh, FROZEN.
#
# The last pure-sh version, kept as the specification of what the C module
# (modules/serie.c) must do: test/unit/module_c_parity.sh runs both under
...
case "$(_pkgmap_one serie)" in
    # The original sourced modules/rust.sh; that module is C now, so the
    # reference points at rust's own frozen copy - same text, still runnable.
    cargo:*) . "$OSR_ROOT/test/ref/rust_sh_ref.sh" ;;
esac
