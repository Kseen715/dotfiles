/* test/unit_c/winbin_test.c -- lib/winbin.c's asset selection.
 *
 ...
 * another platform's build. Both functions are pure, so all of it runs on
 * any host with no network.
 */
#include "../c_test.h"
#include "../../lib/winbin.h"

static void test_match_literals(void) {
    osr_t_true("match: identical strings", osr_winbin_match("a.zip", "a.zip"));
    osr_t_true("match: different strings do not", !osr_winbin_match("a.zip", "b.zip"));
    osr_t_true("match: is anchored at the end",
               !osr_winbin_match("posh-windows-amd64.exe", "posh-windows-amd64.exe.sha256"));
    osr_t_true("match: is anchored at the start",
               !osr_winbin_match("windows-amd64.zip", "fastfetch-windows-amd64.zip"));
    osr_t_true("match: is case sensitive", !osr_winbin_match("A.zip", "a.zip"));
}
...
static void test_match_star(void) {
    ...
    osr_t_true("match: PowerShell asset",
               osr_winbin_match("PowerShell-*-win-x64.zip", "PowerShell-7.6.4-win-x64.zip"));
    osr_t_true("match: WezTerm asset",
               osr_winbin_match("WezTerm-windows-*.zip",
    ...
    osr_t_true("match: x64 pattern rejects the arm64 asset",
               !osr_winbin_match("PowerShell-*-win-x64.zip", "PowerShell-7.6.4-win-arm64.zip"));
    osr_t_true("match: zip pattern rejects the msi",
               !osr_winbin_match("PowerShell-*-win-x64.zip", "PowerShell-7.6.4-win-x64.msi"));
    ...
    osr_t_true("match: * does not span a required literal tail",
               !osr_winbin_match("PowerShell-*-win-x64.zip", "PowerShell-7.6.4-win-fxdependent.zip"));

    osr_t_true("match: leading star", osr_winbin_match("*.zip", "anything.zip"));
    osr_t_true("match: trailing star", osr_winbin_match("starship-*", "starship-x86_64.zip"));
    osr_t_true("match: bare star matches everything", osr_winbin_match("*", "whatever"));
    osr_t_true("match: consecutive stars behave",
               osr_winbin_match("a**b", "axxxb"));
    /* Backtracking: the first candidate for `*` is wrong and it must retry. */
    osr_t_true("match: backtracks past a false start",
               osr_winbin_match("*-win-x64.zip", "PowerShell-7.6.4-win-preview-win-x64.zip"));
}
...
static void test_pick_asset(void) {
    ...
    osr_t_true("pick: finds the x64 zip",
               osr_winbin_pick_asset(RELEASE_JSON, "PowerShell-*-win-x64.zip", url, sizeof(url)));
    ...
    osr_t_true("pick: finds the arm64 zip",
               osr_winbin_pick_asset(RELEASE_JSON, "PowerShell-*-win-arm64.zip", url, sizeof(url)));
    ...
    osr_t_true("pick: no match returns 0",
               !osr_winbin_pick_asset(RELEASE_JSON, "*-freebsd-*.zip", url, sizeof(url)));
    osr_t_eq_str("pick: no match leaves the output empty", url, "");

    osr_t_true("pick: empty payload returns 0",
               !osr_winbin_pick_asset("", "*.zip", url, sizeof(url)));
    osr_t_true("pick: payload with no assets returns 0",
               !osr_winbin_pick_asset("{\"message\":\"Not Found\"}", "*.zip", url, sizeof(url)));
    ...
    osr_t_true("pick: pattern does not match against the URL path",
               !osr_winbin_pick_asset(RELEASE_JSON, "*PowerShell/releases*", url, sizeof(url)));
}

/* --- spec parsing: kind, installer switches, and what remains ------------- */

static void check_spec(const char *label, const char *spec, osr_winbin_kind expect_kind,
    ...
    osr_t_true(buf, osr_winbin_parse_spec(spec, &kind, args, sizeof(args),
...
static void test_parse_spec(void) {
    ...
    check_spec("bare URL", "https://h/f.zip", OSR_WINBIN_KIND_AUTO, "", "https://h/f.zip");
    check_spec("bare gh spec", "gh:o/r:f-*.zip", OSR_WINBIN_KIND_AUTO, "", "gh:o/r:f-*.zip");

    check_spec("explicit zip", "zip:https://h/download", OSR_WINBIN_KIND_ZIP, "",
               "https://h/download");
    check_spec("explicit exe", "exe:https://h/tool", OSR_WINBIN_KIND_EXE, "", "https://h/tool");
    check_spec("explicit msi", "msi:https://h/f.msi", OSR_WINBIN_KIND_MSI, "", "https://h/f.msi");
    check_spec("kind in front of a gh spec", "msi:gh:o/r:f-*.msi", OSR_WINBIN_KIND_MSI, "",
               "gh:o/r:f-*.msi");
    ...
    check_spec("setup with one switch", "setup,/S:https://h/setup.exe",
               OSR_WINBIN_KIND_SETUP, "/S", "https://h/setup.exe");
    check_spec("setup with several switches", "setup,/VERYSILENT,/NORESTART:https://h/s.exe",
               OSR_WINBIN_KIND_SETUP, "/VERYSILENT /NORESTART", "https://h/s.exe");
    check_spec("setup with no switches", "setup:https://h/s.exe",
               OSR_WINBIN_KIND_SETUP, "", "https://h/s.exe");

    osr_t_true("spec: empty is rejected",
               !osr_winbin_parse_spec("", &kind, args, sizeof(args), source, sizeof(source)));
    osr_t_true("spec: a kind with no source is rejected",
               !osr_winbin_parse_spec("zip:", &kind, args, sizeof(args), source, sizeof(source)));
}

static void test_kind_of_file(void) {
    osr_t_eq_int("kind: .zip", osr_winbin_kind_of_file("a.zip"), OSR_WINBIN_KIND_ZIP);
    osr_t_eq_int("kind: .msi", osr_winbin_kind_of_file("PowerShell-7.6.4-win-arm64.msi"),
                 OSR_WINBIN_KIND_MSI);
    osr_t_eq_int("kind: .exe is the program, not an installer",
                 osr_winbin_kind_of_file("posh-windows-amd64.exe"), OSR_WINBIN_KIND_EXE);
    osr_t_eq_int("kind: uppercase extension still recognised",
                 osr_winbin_kind_of_file("TOOL.ZIP"), OSR_WINBIN_KIND_ZIP);
    osr_t_eq_int("kind: unknown extension is AUTO (caller must be told)",
                 osr_winbin_kind_of_file("release-notes.txt"), OSR_WINBIN_KIND_AUTO);
    osr_t_eq_int("kind: no extension is AUTO",
                 osr_winbin_kind_of_file("download"), OSR_WINBIN_KIND_AUTO);
}
