...
#ifndef _WIN32
/* install_self_shim -- put `osr` itself on PATH.
 *
 * Everything that drives this harness from outside a terminal spells it as a
 * bare command: i3's `bindsym $mod+Shift+w exec osr wallpaper --next`, the
 * picker's actions (proteus.toml runs `osr theme {}`), a .desktop entry. None
 * of those go through a login shell and none of them can know where the
 * checkout is, so unless `osr` IS on PATH they die with
 *
 *     proteus: osr: No such file or directory (os error 2)
 *
 * which looks like a broken keybinding rather than a harness that never
 * installed its own front end. Nothing else did this: the launcher is a file
 * in the checkout, and a checkout is not on anybody's PATH.
 *
 * A SHIM, NOT A SYMLINK. The launcher resolves the tree from `dirname $0`, and
 * a symlink at ~/.local/bin/osr gives it a $0 whose directory has no lib/ in
 * it -- which is exactly the signal it reads as "detached", so it would clone
 * a SECOND copy of the repo and drive that instead of the checkout you
 * installed from. Exec'ing through a shim keeps $0 inside the tree.
 *
 * /usr/local/bin, NOT ~/.local/bin. ~/.local/bin is on PATH only because the
 * zsh and xprofile layers put it there, which makes the front end of the
 * harness depend on two modules of the harness having been installed first:
 * on a box whose shell os-rice does not own, `osr` is still not found, and the
 * binding still looks broken. /usr/local/bin is on every PATH there is -- login
 * shell or not, /bin/sh or not, this user or another -- which is what "on PATH"
 * has to mean for a command a keybinding names. Same place i3.c puts osr-term,
 * for the same reason. Termux has no /usr/local and no root, so there it is
 * $PREFIX/bin, which is that system's equivalent.
 *
 * Rewritten every run rather than seeded (§5): it is generated, it holds one
 * path, and that path moves when the checkout moves.
 *
 * Windows has no equivalent here - osr.ps1/osr.bat are the front end there and
 * a source-built binary's directory is put on PATH by lib/build.c.
 */
static void install_self_shim(void) {
    Str launcher, dst, text;
    int termux = osr_on_termux();

    str_initv(&launcher, &dst, &text, (Str *)NULL);
    str_addzz(&launcher, osr_root(), "/osr", (const char *)NULL);
    /* No launcher in the tree: a test sandbox, or the tree a released binary
     * cloned for its files. Neither has a script worth pointing a shim at. */
    if (!file_exists(str_text(&launcher))) goto out;

    if (termux)
        str_addzz(&dst, env_str("PREFIX", "/data/data/com.termux/files/usr"),
                  "/bin/osr", (const char *)NULL);
    else
        str_addz(&dst, "/usr/local/bin/osr");

    str_addzz(&text,
        "#!/bin/sh\n"
        "# Generated by os-rice - do not edit, every install rewrites it.\n"
        "# `osr` on PATH for the callers that have no login shell to find it\n"
        "# with: i3 bindings, proteus, .desktop files. Edit the checkout.\n"
        "exec \"", str_text(&launcher), "\" \"$@\"\n", (const char *)NULL);

    /* Termux is one uid with no root at all; everywhere else this is a system
     * path, so it is written as root and owned by root - a shim on every
     * user's PATH that any user could rewrite is a privilege escalation. */
    if (termux) {
        if (osr_write_user(str_text(&dst), str_text(&text)))
            (void)osr_chmod("0755", str_text(&dst), 0);
    } else if (osr_write_root(str_text(&dst), str_text(&text))) {
        (void)osr_chmod("0755", str_text(&dst), 1);
    }

out:
    str_freev(&launcher, &dst, &text, (Str *)NULL);
}
#endif
...
static int cmd_run(int argc, char **argv) {
    ...
    /* `osr` on PATH, before the modules run: a module (or a config layer one
     * of them writes) that names the command gets one that resolves. */
#ifndef _WIN32
    install_self_shim();
#endif
