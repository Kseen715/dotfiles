/* lib/wallpaper.c -- the C behind wallpaper.sh: the front end that sets or
 * queries the wallpaper of the current theme (DESIGN section 6a).
 *
 ...
 *
 * There is no wallpaper logic of its own here at all: install, record,
 * set-live, library and choose are lib/config.c's, the same functions the sh
 * original called through lib/config.sh, and the only one of them that has
 * two bodies is the painting itself. What is here is exactly what
 * wallpaper.sh was -- an option loop, the current-theme resolution, and four
 * small actions over that library -- which is why it is its own command
 * rather than a mode of `osr install`: this is not an install.
 *
 * It was lib/wallpaper_front.c while a second, Windows-only wallpaper unit
 * held this name.
 *
...
int osr_wallpaper_main(int argc, char **argv) {
    ...
    /* set */
    if (!osr_path_taken(target)) osr_die("no such file: %s", target);
    {
        Str installed;
        str_init(&installed);
