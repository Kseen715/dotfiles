/* modules/datagrip.c -- DataGrip, JetBrains' database IDE, from the vendor
...
static int build_datagrip(void *ctx) {
    (void)ctx;
    return osr_build_run("provide_datagrip");
}

int osrm_datagrip(void) {
    int ok = osr_step("Installing DataGrip", build_datagrip, NULL);
    osr_info("DataGrip installed to /opt/datagrip - run 'datagrip' or use the menu "
             "entry; the first launch asks for a license");
    return ok;
}
