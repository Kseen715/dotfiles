...
if [ -f "$OSR_DOTFILES/micro/settings.json" ]; then
    _mi_frag=""
    if [ -n "${OSR_RICE_DIR:-}" ] && [ -f "$OSR_RICE_DIR/config/micro/settings.json" ]; then
        _mi_frag="$OSR_RICE_DIR/config/micro/settings.json"
    fi
...
if [ -n "${OSR_RICE_DIR:-}" ]; then
    for _mi_cs in "$OSR_RICE_DIR"/config/micro/*.micro; do
        [ -f "$_mi_cs" ] || continue
