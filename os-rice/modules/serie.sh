...
# covers a rice that ships none. In --module mode OSR_RICE_DIR is whatever rice
# the theme picker resolved (§6).
if [ -n "${OSR_RICE_DIR:-}" ] && [ -f "$OSR_RICE_DIR/config/serie/config.toml" ]; then
    install_layer "$OSR_RICE_DIR/config/serie/config.toml" "$OSR_HOME/.config/serie/config.toml"
